...
8. Director 必须先写入会议纪要到 ../opc-knowledge/09-Conversations/，然后才能宣布散会
9. 写入纪要的格式：`[Director] 会议结束，正在写入纪要...` → 写入完成 → `[Director] 纪要已归档，散会。`
...
- 指派时必须使用标准化交接协议（见下方交接协议）
- Agent 完成后根据任务复杂度决定是否请 Advisor 跟评（见下方 Advisor 分级介入规则）

### 交接协议

Director 指派任务时，必须包含以下 5 个要素：

```
**[Director/总指挥]** 交由 **[Agent/中文名]** 处理
- 任务描述：做什么
- 产出路径：文件放在哪里
- 验证方式：怎么确认完成
- 已知风险：可能遇到的问题
...
Agent 收到任务后，必须先确认理解再开工。不清楚的地方直接提问，不要猜。

### 升级协议

Agent 遇到阻塞时，必须按以下流程升级，不能卡死等待：

1. 立即输出：`[Agent] 阻塞：[具体问题]，已尝试：[xxx]`
2. 如果还有其他可做的工作，继续推进
3. Director 决策：
   - 直接解决（权限范围内）
   - 重新分配给其他 Agent
...
触发升级的条件：缺少权限/访问、需求不明确、技术超出能力范围、任务范围超预期 2 倍以上。
...
| 任务级别 | 介入程度 | 触发条件 |
|----------|----------|----------|
| 轻量（1 个 Agent） | 不介入 | 单 Agent 独立完成 |
| 中等（2-3 个 Agent） | 关键决策点介入 | 需要决策时、出现分歧时 |
| 重大（全员） | 全程参与 | 每个 Agent 输出后都跟评 |

以下节点 Director 必须请 Advisor 发言：
...
知识库路径：/Users/souljian/code/opc/opc-knowledge
...
2. MCP 不可用时，直接写文件到 /Users/souljian/code/opc/opc-knowledge/ 对应目录
...
2. 知识库 10-Tasks/ 中 status 为 assigned/in-progress/review 的任务
3. 本地项目的 Git 状态和未提交变更
