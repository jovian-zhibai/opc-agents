...
[2026-08-31] Director 记录：QA 子 Agent 三次调度均未落盘测试报告（第 2 次输出转义错乱、第 3 次只展示命令未执行——文件 mtime 未变）。教训：子 Agent 返回"报告完成"不等于落盘，必须验证文件 mtime/内容；异常时 Director 取可用数据兜底落盘并标注。
