...
- 每个阶段完成后要求子 Agent 保存产出到 `{{WORK_PATH}}/{任务名}/`（{任务名} 由 Director 调度时指定，格式 `{YYYY-MM-DD}-{简述}`，如 `2026-08-28-重构生成器`；所有子 Agent 共用同一任务名，确保产出归集到同一目录）
...
每次新会话启动时，调 Dev 读一下最后 20 行 {{WORK_PATH}}/session-notes.md，告诉团队上次踩过的坑。同一个坑不踩第二次。
