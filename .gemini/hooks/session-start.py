#!/usr/bin/env python3
"""Gemini CLI SessionStart hook: 会话启动自动检查（五运行时共享核心适配）。

   Gemini CLI 在会话启动时触发本 hook（v0.26.0+ 默认启用 hooks）。
   hook 从 stdin 读 JSON（Gemini CLI hook 输入），输出 {"additionalContext": "..."}
   注入给模型——与 Claude Code 的 hookSpecificOutput.additionalContext 等价。
   ...
   官方文档：https://geminicli.com/docs/hooks/writing-hooks/
"""
import json
import os
import subprocess
...
def main():
    # Gemini CLI hook 协议：stdin 读 JSON（可能为空或不完整）
    try:
    ...
    # 如果 Gemini CLI 传入了 prompt（如 resume 时），则用 user_prompt 模式
    ...
    try:
        ...
        # 输出 Gemini CLI hook 格式：{"additionalContext": "..."}
        sys.stdout.write(json.dumps({"additionalContext": context}, ensure_ascii=False))
        sys.stdout.flush()
